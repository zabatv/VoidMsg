...
export const api = {
  ...
  avatarUrl: (userId) => API_URL + '/users/' + userId + '/avatar?token=' + encodeURIComponent(getToken() || ''),
  uploadAvatar: (file) => {
    const fd = new FormData();
  ...
  postImageUrl: (id) => API_URL + '/posts/' + id + '/image?token=' + encodeURIComponent(getToken() || ''),
